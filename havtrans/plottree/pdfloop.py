...
tree1 <- read.newick('%s.mp.treefile')
tree <- read.newick('%s.mp_clusterPicks.nwk')
...
for(cluster_n in 1:length(list_of_clusters)){
    ...
    ggtree_tre <- ggtree(tre, branch.length = 'none') +
        ...
        q <- msaplot(p=ggtree_tre, fasta='%s', bg_line = FALSE, width = 100, offset = 10) 
        plot_list[[cluster_n]] <- q
    }

dev.off()
pdf(file = paste('%s', '.mp.treefile.msa.pdf', sep=''), paper = 'a4r', width=11.69, height=8.27, onefile = TRUE)
for(i in plot_list){
    print(i)
}
dev.off()
