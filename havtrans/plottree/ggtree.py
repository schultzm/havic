...
q <- p + geom_tiplab(aes(label=label, color=Cluster), size=1, linesize=0.2, align=TRUE) +
    ...
    annotate("text", x = 0.015, y=-4, label = "Substitutions per site", size=1) + ggtitle(label = "ML IQtree with bootstrap %, tips cluster-picked (left); fasta alignment (right)", subtitle = paste0('Picked clusters have allowed ', nsnps, ' SNPs in ', seqlen, ' bp divergence'))
